...
def get_save_root():
    ...
    return  os.path.join("assets","res",  dataset_dir)
...
def eval_triplet_network(support_size):
    ...
    for surface in os.listdir(os.path.join("assets", "input", dataset_dir)):
    # for surface in ["left","wall","base"]:

        print("eval surface: " + surface,end=";  ")
        net.load_state_dict(torch.load(net_dir))
        paired_testdata = pair_data.load_pair_test_dataset(os.path.join("assets", "input", "cjy"), surface, support_size)
